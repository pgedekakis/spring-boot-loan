...
public interface PaymentActionsRepository extends JpaRepository<PaymentActions, Long> {
    ...
    @Query(value = "SELECT instalmentActions FROM InstalmentActions instalmentActions WHERE instalmentActions.paymentActions.id=:instalmentActionsId")
    InstalmentActions getInstalmentActionsIds(@Param("instalmentActionsId") Long instalmentActionsId);
